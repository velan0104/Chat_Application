...
import { EXIT_CHANNEL_ROUTE, GET_ALL_MEMBERS_ROUTE, GET_ALL_NEW_MEMBER_ROUTE, HOST, REMOVE_CHANNEL_MEMBER_ROUTE, RENAME_GROUP_ROUTE } from '@/utils/constants';
import EmojiPicker from 'emoji-picker-react';
import moment from 'moment';
import { useEffect, useRef, useState } from 'react';
import { FaArrowLeftLong, FaPen } from "react-icons/fa6";
import { IoPersonAddOutline } from "react-icons/io5";
import { MdDelete, MdExitToApp, MdOutlineDone, MdOutlineEmojiEmotions } from "react-icons/md";
...
const ChatDetails = () =>{
  const { setIsChatDetails, selectedChatData, userInfo, setAllChannelMembers, allChannelMembers } = useAppStore();
  const { toast } = useToast()
  const emojiRef = useRef();
  // console.log("Receiver: ", selectedChatData)
  ...
  useEffect(() => {
    const getData = async() => {
      const response = await apiClient.get(`${GET_ALL_MEMBERS_ROUTE}/${selectedChatData._id}`, {
    ...
    getData()
    console.log("Members:" , allChannelMembers)
  }, [])
  ...
  const Button = ({content, disabled = false, event, className = ""}) => {
    return (
      <button
        className={`cursor-pointer p-3 hover:bg-slate-600 rounded-full ${disabled && "text-gray-700"} ${className}`}
        onClick = {event}
        disabled = {disabled}
      >
        {content}
  ...
  const handleChanges = () => {
    const response = apiClient.put(RENAME_GROUP_ROUTE, {
  ...
  return (
    <div className='scroll-bar overflow-y-auto'>
      <div
        className='h-[10vh] border-b-2 border-[#2f303b] flex items-center justify-between px-5'
      >
        <button 
      ...
      <div className='flex justify-center'>
        <AlertDialog>
          <AlertDialogTrigger>
            <div
              className='w-[500px] bg-red-500 p-3 rounded-lg flex items-center justify-center m-5 gap-x-1 hover:bg-red-700'
            >
          ...
          <AlertDialogContent className="bg-[#121213] border-none">
            ...
            <AlertDialogFooter>
              ...
              <AlertDialogAction onClick={handleExit} className="bg-red-700 hover:bg-red-600">Exit</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
